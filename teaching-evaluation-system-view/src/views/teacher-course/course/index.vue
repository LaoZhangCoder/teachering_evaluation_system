<template>
  <div class="app-container">
    <el-button type="primary" @click="handleAddRole">进行选课</el-button>
    ...
    <el-dialog :visible.sync="dialogVisible" :title="dialogType==='edit'?'Edit Course':'New Course'">
      <el-form :model="course" label-width="80px" label-position="left">
        ...
        <el-form-item label="所在班级">
          <el-select v-model="course.classId" placeholder="所在班级" @change="classChange(course.classId)">
            <el-option v-for="major in classes" :label="major.className" :value="major.classId" />
          </el-select>
        </el-form-item>
        <el-form-item label="所教课程">
          <el-select v-model="course.courseId" placeholder="所在课程">
            <el-option v-for="subject in subjects" :label="subject.courseName" :value="subject.id" />
          </el-select>
        </el-form-item>
      </el-form>
...
export default {
  ...
  methods: {
    ...
    classChange(classId) {
      this.subjects = null
      queryCourseByCondition(classId).then(response => {
